#!/usr/bin/env python3
"""
Script to insert busking location data into the database
"""
...
async def insert_location_data():
    ...
    print("🏢 Inserting busking location data...")
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            for data in LOCATION_DATA:
                location = BuskingLocation(**data)
                locations.append(location)
            
            db.add_all(locations)
            await db.commit()
            
            print(f"✅ Successfully inserted {len(locations)} busking locations!")
            ...
            print(f"❌ Failed to insert location data: {str(e)}")
            await db.rollback()
            raise

if __name__ == "__main__":
    asyncio.run(insert_location_data())
